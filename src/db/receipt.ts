...
export const addReceipt = async (db: NitroSQLiteConnection, uri: string, date: number) => {
    const query: string = "INSERT INTO receipt (image_uri, receipt_date) VALUES (?, ?);";
    try {
        const result: QueryResult<QueryResultRow> = await db.executeAsync(query, [uri, date.toString()]);
