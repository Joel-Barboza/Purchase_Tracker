...
export const createProductTable = async (db: NitroSQLiteConnection): Promise<void> => {
  const productsQuery = `
...
export const addProduct = async (
  db: NitroSQLiteConnection, product: Product
  ...
  try {
    const result: QueryResult<QueryResultRow> = await db.executeAsync(query, [name, prodCode ?? NITRO_SQLITE_NULL, unitPrice, soldByKg]);
...
export const updateProductPrice = async (
  db: NitroSQLiteConnection, productCode: string, newPrice: number
): Promise<void> => {
...
export const findProductByCode = async (
  db: NitroSQLiteConnection, productCode: string
): Promise<QueryResultRow | undefined> => {

  const query: string = "SELECT * FROM product WHERE product_code = ?;";
  try {
    const result: QueryResult<QueryResultRow> = await db.executeAsync(query, [productCode]);
...
export const getAllProducts = async (db: NitroSQLiteConnection) => {
  try {
    const result: QueryResult<QueryResultRow> = await db.executeAsync(`SELECT * FROM product;`);
    if (!result.rows) return;
    if (result.rows.length > 0) {
      return result.rows;
    } else {
